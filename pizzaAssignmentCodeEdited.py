...
def select_pizza(total_cost, receipt):
    ...
    if toppings == []:
        ...
        receipt_output = 'Your pizza costs \xA3' + str(cost) + '. It is a ' + pizza_size + ' pizza with no toppings'
        generate_receipt("testreceipt.txt",receipt_output)

    else: 
        print('Your pizza costs \xA3' + str(cost) + '. It is a ' + pizza_size + ' pizza with ' + str(toppings))
        

        receipt_output = 'Your pizza costs \xA3' + str(cost) + '. It is a ' + pizza_size + ' pizza with ' + str(toppings)
        generate_receipt("testreceipt.txt",receipt_output)


def generate_receipt(receipt_name,receipt_detail):
    output = open(receipt_name+'.txt', 'a')
    output.write(receipt_detail+"\n")
    output.close()
